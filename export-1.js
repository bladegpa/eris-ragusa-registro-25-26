...
async function buildDocxPagella(st,idx){
  ...
  const annoLabel=annoN===1?"I annualit\u00e0":annoN===2?"II annualit\u00e0":annoN===3?"III annualit\u00e0":"IV annualit\u00e0";
...
function buildHtmlPagella(st,idx){
  ...
  const righeHtml=subjCols.map(s=>{
    const e=App.grades[s.id]?.[idx];
    const voto=e?e.value:"—";
    const nomeModulo=s.label.replace(/^[A-Za-z0-9]+[Eeb]* - /,"").toUpperCase();
    ...
    return`<tr>
      <td style="padding:2.5px 6px;font-size:8.5pt;border:0.5pt solid #CBD5E1;vertical-align:middle">${nomeModulo}</td>
      <td style="padding:2.5px;font-size:8pt;border:0.5pt solid #CBD5E1;text-align:center;color:#475569;vertical-align:middle">${s.ore>0?s.ore:"—"}</td>
      <td style="padding:2.5px;font-size:9.5pt;font-weight:800;border:0.5pt solid #CBD5E1;text-align:center;color:${vColor};background:${vBg};vertical-align:middle">${voto}</td>
    </tr>`;
  }).join("");
...
<table class="tb" style="margin-bottom:5px"><tr><td style="padding:4px 8px">
  ...
  <div style="flex:1.6">
    <table class="tg" style="width:100%;border-collapse:collapse">
      <tr style="background:#0F2557;color:white">
        <th style="padding:3px 6px;font-size:8pt;text-align:left;border:0.5pt solid #0F2040">MODULO</th>
        <th style="padding:3px;font-size:8pt;text-align:center;border:0.5pt solid #0F2040;width:28px">ORE</th>
        <th style="padding:3px;font-size:8pt;text-align:center;border:0.5pt solid #0F2040;width:34px">VOTO</th>
      </tr>
      ${righeHtml}
    </table>
  </div>
  <div style="flex:1;border:1pt solid #CBD5E1;padding:8px 10px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;background:#FAFAFA">
    <div style="font-size:9.5pt;font-weight:800;letter-spacing:0.5px">SI ATTESTA CHE</div>
    <div style="font-size:8.5pt">L&apos;allievo</div>
    <div style="font-size:10pt;font-weight:900;color:#0F2557">${alunno}</div>
    <div style="font-size:8.5pt">è stato <strong><u>AMMESSO</u></strong> alla</div>
    <div style="font-size:8.5pt;font-weight:800">${annoLabel} con il seguente giudizio finale:</div>
    <div style="font-size:15pt;font-weight:900;color:${giudizioColor(giudizio)};margin:3px 0">${giudizio}</div>
    <div style="font-size:7.5pt;color:#64748B">Condotta:</div>
    <div style="font-size:15pt;font-weight:900;color:${gradeColor(votoCond)}">${votoCond}</div>
  </div>
</div>
<p class="giudizio" style="margin-top:10px"><strong>VOTAZIONE FINALE</strong>:&emsp;<span style="font-size:22pt;font-weight:900;color:${mpColor}">${mpx10!==null?mpx10+" / 100":"—"}</span></p>
<div class="firme"><span><strong>Data</strong>: Ragusa, ${new Date().toLocaleDateString("it-IT")}</span><span><strong>La Segreteria</strong>: ___________________________</span></div>
</div>
<div class="pf"><img class="fi" src="${IMG_ASSET_1}" alt="ISO 9001"><div class="ft"><b>ERIS ENTE DEL TERZO SETTORE</b>Sede legale: via Salvatore Paola, 14/a &ndash; 95125 Catania | tel./fax: 095433940 | didattica.ct@erisformazione.it | amministrazione.ct@erisformazione.it<br>Associazione riconosciuta, iscrizione n&deg;&nbsp;293979 C.C.I.A.A. di Catania | CF: 97180200822 | info@pec.erisformazione.it | www.erisformazione.it</div><img class="fi" src="${IMG_ASSET_2}" alt="OHSAS 18001"></div>
</div>
</div>`;
